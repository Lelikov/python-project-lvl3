...
def parser(dom, url, name, output):
    for attribute in ATTRIBUTES:
        ...
        for tag in dom.find_all(**param):
            if tag.name == 'a':
                tag[attribute] = url_normalization(tag[attribute], url)
            else:
                normal_url = url_normalization(tag[attribute], url)
                ...
                tag[attribute] = name + PATH + changed_filename + file_extension
